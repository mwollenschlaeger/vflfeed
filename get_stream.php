...
function to_html($category, $title, $content, $link, $date, $favicon) {
	...
	$r = <<<OUTPUT
	<article class="social-item {$category}" data-category="{$category}">
	  <header>
	  	<pre class="{$category}"> <a href="http://{$category}.de" title="{$category}">{$category}</a></pre>
	  	<h1><a href="{$link}" title="{$title}">{$title}</a></h1>
	  	<span><time datetime="{$date}" pubdate>{$date}</time></span>
	  </header>
    	<div class="truncate">{$content}</div>
    	<a href="{$link}" title="Weiterlesen »" class="read-more">Weiterlesen »</a>
    </article>
OUTPUT;
...
function update_DB($apis, $mysqli, $mysql_table) {
	...
	$feed->set_cache_duration(4000);	// 15 minute cache
	$feed->init();
	...
	foreach($feed->get_items() as $item) {
		...
		if ($stmt->execute()) {
			...
			echo to_html($category, $title, $content, $link, $date, $favicon);
		}
	}
